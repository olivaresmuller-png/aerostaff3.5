...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, Users, Clock, Target, CheckCircle, XCircle, AlertTriangle, Search, User, GraduationCap, Printer, ClipboardList } from 'lucide-react';
...
export function CalendarPage() {
  const { employees, assignments, selectedDate, setSelectedDate, updateAssignment, getAssignment, getWeekDates, importMetadata } = useAppStore();
  ...
  const dayStats = useMemo(() => {
    ...
    const offDuty = dayAssignments.filter(a => !isOnDuty(a.shiftCode)).length;
  ...
  const canGrantOffDay = useMemo(() => {
    ...
    const analyzeDay = (dateStr: string) => {
      ...
      const offDutyCount = dayAssignments.filter(a => !isOnDuty(a.shiftCode)).length;
                          ...
                          <TableCell>
                            {emp.ftCount > 0 ? (
                              <div className="flex flex-wrap gap-1">
                                {emp.ftDates.slice(0, 5).map(date => (
                                  <Badge key={date} variant="outline" className="text-xs">
                                    {format(new Date(date), 'MMM d')}
                                  </Badge>
                                ))}
                                {emp.ftDates.length > 5 && (
                                  <Badge variant="outline" className="text-xs text-muted-foreground">
                                    +{emp.ftDates.length - 5} more
                                  </Badge>
                                )}
                              </div>
                            ) : (
        ...
        <Card className="p-4 bg-card border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-display font-bold">{DAILY_TARGET}h</p>
            </div>
          </div>
        </Card>
