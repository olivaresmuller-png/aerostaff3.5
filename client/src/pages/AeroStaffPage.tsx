...
import { Priority, SpecialSkill, Aircraft, DutyStatus, WorkRequirement, Assignment, Employee, ShiftCode, DayAssignment } from '@/types/aerostaff';
import { Header } from '@/components/aerostaff/Header';
import { StatsCards } from '@/components/aerostaff/StatsCards';

const SPECIAL_SKILLS: SpecialSkill[] = [
  'FUEL TANK', 'Walliclean', 'Forklift', 'CYCLEAN', 'Cobra', 'Cee Bee',
  'OXY Hand', 'ENTRY', 'A32CFM Boro', 'A32PW Boro', 'A330RR Boro',
  'A340 Boro', 'A350 Boro', 'A220 Boro', 'B777 Boro',
...
const SKILL_LABELS: Record<SpecialSkill, { label: string; color: string }> = {
  ...
  'OXY Hand': { label: 'OXY Hand', color: 'bg-blue-400' },
  'ENTRY': { label: 'Entry', color: 'bg-green-600' },
  'A32CFM Boro': { label: 'A32 CFM Boro', color: 'bg-purple-500' },
  'A32PW Boro': { label: 'A32 PW Boro', color: 'bg-purple-600' },
  'A330RR Boro': { label: 'A330 RR Boro', color: 'bg-purple-700' },
  'A340 Boro': { label: 'A340 Boro', color: 'bg-purple-800' },
  'A350 Boro': { label: 'A350 Boro', color: 'bg-purple-900' },
  'A220 Boro': { label: 'A220 Boro', color: 'bg-fuchsia-500' },
  'B777 Boro': { label: 'B777 Boro', color: 'bg-fuchsia-700' },
  'A220 RU': { label: 'A220 RU', color: 'bg-pink-500' },
  'A320 RU': { label: 'A320 RU', color: 'bg-pink-600' },
  'A320NEO RU': { label: 'A320 NEO RU', color: 'bg-pink-400' },
  'A330 RU': { label: 'A330 RU', color: 'bg-pink-700' },
  'A343 RU': { label: 'A343 RU', color: 'bg-pink-800' },
  'A350 RU': { label: 'A350 RU', color: 'bg-pink-900' },
  'B777 RU': { label: 'B777 RU', color: 'bg-pink-300' },
};
...
export function AeroStaffPage() {
  ...
  const assignableEmployees = useMemo(() =>
    employees.filter((emp: Employee) => emp.role !== 'PM' && emp.role !== 'PS'),
    [employees]
  );
  ...
  const calendarDutyStatus = useMemo<{ employeeId: string; onDuty: boolean; shiftCode: ShiftCode }[]>(() => {
    return calendarAssignments.filter((a: DayAssignment) => a.date === dateStr).map((a: DayAssignment) => ({
  ...
  useEffect(() => {
    const existingDuty = dutyStatuses.filter((d: DutyStatus) => d.date === dateStr);
    if (existingDuty.length === 0) {
      const newDutyStatuses = employees.map((emp: Employee) => {
        const calStatus = calendarDutyStatus.find((c: { employeeId: string; onDuty: boolean; shiftCode: ShiftCode }) => c.employeeId === emp.id);
        return { date: dateStr, employeeId: emp.id, onDuty: calStatus?.onDuty ?? true };
      });
      setDutyStatuses((prev: DutyStatus[]) => [...prev, ...newDutyStatuses]);
    }
  }, [dateStr, employees, calendarDutyStatus]);

  const todaysDuty = useMemo(() => dutyStatuses.filter((d: DutyStatus) => d.date === dateStr), [dutyStatuses, dateStr]);
  const todaysRequirements = useMemo(() => workRequirements.filter((r: WorkRequirement) => r.date === dateStr), [workRequirements, dateStr]);
  const todaysAssignments = useMemo(() => assignments.filter((a: Assignment) => a.date === dateStr), [assignments, dateStr]);

  const aircraftAssignmentsMap = useMemo(() => {
    const map: Record<string, string> = {};
    todaysAssignments.forEach((assignment: Assignment) => {
      const ac = aircraft.find((a: Aircraft) => a.id === assignment.aircraftId);
      if (ac) {
  ...
  const onDutyEmployees = useMemo(() => assignableEmployees.filter((emp: Employee) => {
  ...
  const toggleDutyStatus = (employeeId: string) => {
    setDutyStatuses((prev: DutyStatus[]) => prev.map((d: DutyStatus) => d.date === dateStr && d.employeeId === employeeId ? { ...d, onDuty: !d.onDuty } : d));
  };

  const addAircraftByRegistration = () => {
    const reg = registrationInput.trim().toUpperCase();
    if (!reg) return;
    let matchedAircraft = aircraft.find((a: Aircraft) => a.registration.toUpperCase() === reg);
    if (!matchedAircraft) matchedAircraft = aircraft.find((a: Aircraft) => a.registration.toUpperCase().includes(reg));
    if (!matchedAircraft) {
      toast({ title: 'Aircraft Not Found', description: `Registration "${reg}" not found in fleet. Add it first.`, variant: 'destructive' });
      return;
    }
    if (workRequirements.find((r: WorkRequirement) => r.date === dateStr && r.aircraftId === matchedAircraft!.id)) {
    ...
    setWorkRequirements((prev: WorkRequirement[]) => [...prev, newReq]);
  ...
  const removeWorkRequirement = (reqId: string) => {
    const req = workRequirements.find((r: WorkRequirement) => r.id === reqId);
    setWorkRequirements((prev: WorkRequirement[]) => prev.filter((r: WorkRequirement) => r.id !== reqId));
    if (req) setAssignments((prev: Assignment[]) => prev.filter((a: Assignment) => !(a.aircraftId === req.aircraftId && a.date === dateStr)));
  };
  ...
  const updateWorkRequirement = (aircraftId: string, field: 'b1Required' | 'b2Required' | 'catARequired' | 'b1Hours' | 'b2Hours' | 'catAHours' | 'b1TotalHours' | 'b2TotalHours' | 'catATotalHours', value: number) => {
    setWorkRequirements((prev: WorkRequirement[]) => prev.map((r: WorkRequirement) => {
  ...
  const canAssign = (employee: Employee, ac: Aircraft, role: 'B1' | 'B2' | 'Cat A/Helper'): boolean => {
  ...
  const hasSpecialSkill = (employee: Employee, skill: SpecialSkill): boolean => {
    return employee.certifications.some(cert => cert.toLowerCase().includes(skill.toLowerCase()));
  };

  const countMatchingSkills = (employee: Employee, requiredSkills: SpecialSkill[]): number => {
  ...
  const getEmployeeGrade = (emp: Employee): number => {
    return emp.grade || 0;
  };

  const isB12License = (emp: Employee, aircraftType: string): boolean => {
    const skill = emp.skills.find(s => canWorkOnAircraft(s.aircraftType, aircraftType));
    return skill?.license === 'B1/2';
  };

  const isSeniorEngineer = (emp: Employee): boolean => {
    return emp.role === 'SrEng';
  };

  const isCatAOnly = (emp: Employee): boolean => {
  ...
  const runAutoAssignment = () => {
    ...
    const sortedRequirements = [...todaysRequirements].sort((a: WorkRequirement, b: WorkRequirement) => PRIORITY_CONFIG[(a.priority || 'normal') as Priority].order - PRIORITY_CONFIG[(b.priority || 'normal') as Priority].order);

    const isAssignedAnywhere = (empId: string) => newAssignments.some(a => a.employeeId === empId);

    const seEmployees = onDutyEmployees.filter((emp: Employee) => isSeniorEngineer(emp));
    const nonSeEmployees = onDutyEmployees.filter((emp: Employee) => !isSeniorEngineer(emp) && !isCatAOnly(emp));
    const catAEmployees = onDutyEmployees.filter((emp: Employee) => isCatAOnly(emp));

    for (const req of sortedRequirements) {
      const ac = aircraft.find(a => a.id === req.aircraftId);
      if (!ac) continue;

      const existingManual = todaysAssignments.filter((a: Assignment) => a.aircraftId === req.aircraftId && a.isManualOverride);
      existingManual.forEach((a: Assignment) => {
        employeeAssignmentCount[a.employeeId] = (employeeAssignmentCount[a.employeeId] || 0) + 1;
        newAssignments.push(a);
      });

      const b1Manual = existingManual.filter((a: Assignment) => a.role === 'B1').length;
      const b2Manual = existingManual.filter((a: Assignment) => a.role === 'B2').length;
      ...
      const eligibleSE_B1 = shuffleArray(seEmployees.filter((emp: Employee) => {
      ...
      const eligibleSE_B2 = shuffleArray(seEmployees.filter((emp: Employee) => {
    ...
    for (const req of sortedRequirements) {
      ...
      const isAssignedToThisAircraft = (empId: string) => newAssignments.some((a: Assignment) => a.employeeId === empId && a.aircraftId === ac.id);
      const requiredSkills = req.specialSkillsRequired || [];

      const currentB1 = newAssignments.filter((a: Assignment) => a.aircraftId === ac.id && a.role === 'B1').length;
      const currentB2 = newAssignments.filter((a: Assignment) => a.aircraftId === ac.id && a.role === 'B2').length;
  ...
  const getAssignmentStats = () => {
    ...
    todaysRequirements.forEach((req: WorkRequirement) => {
  ...
  const printAssignments = () => {
    const printContent = `
      ...
      ${todaysRequirements.map((req: WorkRequirement) => {
      const ac = aircraft.find((a) => a.id === req.aircraftId);
      if (!ac) return '';
      ...
      return `<div class="aircraft"><div class="aircraft-header">${ac.registration} (${ac.type}) - ${PRIORITY_CONFIG[req.priority].label}</div>
          ...
          <div class="role-section"><div class="role-title">B1 (${b1.length}/${req.b1Required}) <span class="hours">${req.b1Hours || 8}h each</span></div>${b1.map((a: Assignment) => { const emp = employees.find((e: Employee) => e.id === a.employeeId); return `<div class="employee">${emp?.name} (${emp?.initials})</div>`; }).join('')}</div>
          <div class="role-section"><div class="role-title">B2 (${b2.length}/${req.b2Required}) <span class="hours">${req.b2Hours || 8}h each</span></div>${b2.map((a: Assignment) => { const emp = employees.find((e: Employee) => e.id === a.employeeId); return `<div class="employee">${emp?.name} (${emp?.initials})</div>`; }).join('')}</div>
          ${(req.catARequired || 0) > 0 ? `<div class="role-section"><div class="role-title">Cat A/Helper (${catA.length}/${req.catARequired || 0}) <span class="hours">${req.catAHours || 8}h each</span></div>${catA.map((a: Assignment) => { const emp = employees.find((e: Employee) => e.id === a.employeeId); return `<div class="employee">${emp?.name} (${emp?.initials})</div>`; }).join('')}</div>` : ''}</div>`;
    }).join('')}
  ...
  const getAvailableForReassign = () => todaysAssignments.map((a: Assignment) => ({ assignment: a, emp: employees.find((e: Employee) => e.id === a.employeeId), ac: aircraft.find((ac: Aircraft) => ac.id === a.aircraftId) })).filter((item) => item.emp && item.ac);

  const getUnfilledAircraft = (role: 'B1' | 'B2' | 'Cat A/Helper') => todaysRequirements.filter((req: WorkRequirement) => {
    const acAssignments = todaysAssignments.filter((a) => a.aircraftId === req.aircraftId);
  ...
  const setAllDuty = (onDuty: boolean) => {
    const newStatuses = employees.map((emp: Employee) => ({ date: dateStr, employeeId: emp.id, onDuty }));
    setDutyStatuses((prev) => [...prev.filter((d: DutyStatus) => d.date !== dateStr), ...newStatuses]);
  };
  ...
  return (
    <div className="p-6 lg:p-8 space-y-6" data-testid="aerostaff-page">
      <Header
        ...
        onAutoAssign={runAutoAssignment}
      />
        ...
        <Card className="border-red-500/50 bg-red-50 dark:bg-red-950/20" data-testid="card-unfilled-details">
          ...
          <CardContent>
            <div className="space-y-2">
              {todaysRequirements.map((req: WorkRequirement) => {
                const ac = aircraft.find((a: Aircraft) => a.id === req.aircraftId);
                if (!ac) return null;
                const acAssignments = todaysAssignments.filter((a: Assignment) => a.aircraftId === req.aircraftId);
                const b1Missing = Math.max(0, req.b1Required - acAssignments.filter((a: Assignment) => a.role === 'B1').length);
                const b2Missing = Math.max(0, req.b2Required - acAssignments.filter((a: Assignment) => a.role === 'B2').length);
                ...
                return (
                  <div key={req.id} className="flex items-center justify-between p-2 rounded bg-white/50 dark:bg-black/20 border">
                    <div className="flex items-center gap-2">
                      <Badge className={`text-xs text-white ${PRIORITY_CONFIG[(req.priority || 'normal') as Priority].color}`}>{PRIORITY_CONFIG[(req.priority || 'normal') as Priority].label}</Badge>
                    ...
                    <div className="space-y-4 py-4">
                      ...
                      <div className="space-y-2"><Label>Move to aircraft</Label>
                        <Select value={reassignToAircraft} onValueChange={setReassignToAircraft}>
                          ...
                          <SelectContent>{getUnfilledAircraft(reassignRole).map((req: WorkRequirement) => { const ac = aircraft.find((a: Aircraft) => a.id === req.aircraftId); return (<SelectItem key={req.id} value={req.aircraftId}>{ac?.registration} ({ac?.type})</SelectItem>); })}</SelectContent>
                        </Select>
                      </div>
                    </div>
            ...
            <CardContent>
              {todaysRequirements.length === 0 ? (
                ...
                <div className="space-y-6">
                  {[...todaysRequirements].sort((a: WorkRequirement, b: WorkRequirement) => PRIORITY_CONFIG[(a.priority || 'normal') as Priority].order - PRIORITY_CONFIG[(b.priority || 'normal') as Priority].order).map((req: WorkRequirement) => {
                    const ac = aircraft.find((a: Aircraft) => a.id === req.aircraftId);
                    if (!ac) return null;
                    const acAssignments = todaysAssignments.filter((a: Assignment) => a.aircraftId === req.aircraftId);
                    const b1Assigned = acAssignments.filter((a: Assignment) => a.role === 'B1');
                    const b2Assigned = acAssignments.filter((a: Assignment) => a.role === 'B2');
                    const catAAssigned = acAssignments.filter((a: Assignment) => a.role === 'Cat A/Helper');
                    const requiredSkills = req.specialSkillsRequired || [];
                    const assignedEmps = acAssignments.map((a: Assignment) => employees.find((e: Employee) => e.id === a.employeeId)).filter(Boolean);
                    const coveredSkills = requiredSkills.filter((skill) => assignedEmps.some((emp) => emp && hasSpecialSkill(emp as Employee, skill)));
                      ...
                      <div key={req.id} className={cn("p-4 rounded-lg border", req.priority === 'urgent' ? "border-red-500/50 bg-red-50/50 dark:bg-red-950/20" : req.priority === 'high' ? "border-orange-500/50 bg-orange-50/50" : "bg-muted/30")}>
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="flex items-center gap-2"><h3 className="font-display font-semibold text-lg">{ac.registration}</h3><Badge className={`text-xs text-white ${PRIORITY_CONFIG[(req.priority || 'normal') as Priority].color}`}>{PRIORITY_CONFIG[(req.priority || 'normal') as Priority].label}</Badge></div>
                        ...
                        <div className="grid lg:grid-cols-3 gap-4">
                          <div className="space-y-2">
                            ...
                            {b1Assigned.map((a: Assignment) => {
                              const emp = employees.find((e: Employee) => e.id === a.employeeId); const empSkills = emp ? requiredSkills.filter(s => hasSpecialSkill(emp, s)) : []; return (
                            ...
                            {b1Assigned.length < req.b1Required && (
                              <Select onValueChange={(empId) => addManualAssignment(ac.id, empId, 'B1')}>
                                ...
                                <SelectContent>{onDutyEmployees.filter((emp: Employee) => canAssign(emp, ac, 'B1') && !b1Assigned.some((a: Assignment) => a.employeeId === emp.id)).map((emp: Employee) => (<SelectItem key={emp.id} value={emp.id}>{emp.name} ({emp.initials}) {requiredSkills.filter((s: SpecialSkill) => hasSpecialSkill(emp, s)).length > 0 ? `[${requiredSkills.filter((s: SpecialSkill) => hasSpecialSkill(emp, s)).map((s: SpecialSkill) => SKILL_LABELS[s].label).join(', ')}]` : ''}</SelectItem>))}</SelectContent>
                              </Select>
                            )}
                          </div>
                          <div className="space-y-2">
                            <div className="font-medium text-sm text-emerald-600">B2 Avionics</div>
                            {b2Assigned.map((a: Assignment) => {
                              const emp = employees.find((e: Employee) => e.id === a.employeeId); const empSkills = emp ? requiredSkills.filter(s => hasSpecialSkill(emp, s)) : []; return (
                            ...
                            {b2Assigned.length < req.b2Required && (
                              <Select onValueChange={(empId) => addManualAssignment(ac.id, empId, 'B2')}>
                                ...
                                <SelectContent>{onDutyEmployees.filter((emp: Employee) => canAssign(emp, ac, 'B2') && !b2Assigned.some((a: Assignment) => a.employeeId === emp.id)).map((emp: Employee) => (<SelectItem key={emp.id} value={emp.id}>{emp.name} ({emp.initials}) {requiredSkills.filter((s: SpecialSkill) => hasSpecialSkill(emp, s)).length > 0 ? `[${requiredSkills.filter((s: SpecialSkill) => hasSpecialSkill(emp, s)).map((s: SpecialSkill) => SKILL_LABELS[s].label).join(', ')}]` : ''}</SelectItem>))}</SelectContent>
                              </Select>
                            )}
                          </div>
                          <div className="space-y-2">
                            <div className="font-medium text-sm text-amber-600">Cat A / Helpers</div>
                            {catAAssigned.map((a: Assignment) => {
                              const emp = employees.find((e: Employee) => e.id === a.employeeId); const empSkills = emp ? requiredSkills.filter(s => hasSpecialSkill(emp, s)) : []; return (
                            ...
                            {catAAssigned.length < (req.catARequired || 0) && (
                              <Select onValueChange={(empId) => addManualAssignment(ac.id, empId, 'Cat A/Helper')}>
                                ...
                                <SelectContent>{onDutyEmployees.filter((emp: Employee) => !catAAssigned.some((a: Assignment) => a.employeeId === emp.id)).map((emp: Employee) => (<SelectItem key={emp.id} value={emp.id}>{emp.name} ({emp.initials}) {requiredSkills.filter((s: SpecialSkill) => hasSpecialSkill(emp, s)).length > 0 ? `[${requiredSkills.filter((s: SpecialSkill) => hasSpecialSkill(emp, s)).map((s: SpecialSkill) => SKILL_LABELS[s].label).join(', ')}]` : ''}</SelectItem>))}</SelectContent>
                              </Select>
                            )}
              ...
              <Table>
                ...
                <TableBody>
                  {employees.map((emp: Employee) => {
                    const calStatus = calendarDutyStatus.find(c => c.employeeId === emp.id);
                    const localStatus = todaysDuty.find((d: DutyStatus) => d.employeeId === emp.id);
                      ...
                      <div key={req.id} className={cn("p-4 border rounded-lg", req.priority === 'urgent' && "border-red-500/50 bg-red-50/30", req.priority === 'high' && "border-orange-500/50 bg-orange-50/30")}>
                        ...
                        <div className="mb-4">
                          ...
                          <div className="flex gap-2 mt-1">
                            <Badge variant="secondary" className="text-blue-600">B1: {ac ? onDutyEmployees.filter((emp: Employee) => canAssign(emp, ac, 'B1')).length : 0}</Badge>
                            <Badge variant="secondary" className="text-emerald-600">B2: {ac ? onDutyEmployees.filter((emp: Employee) => canAssign(emp, ac, 'B2')).length : 0}</Badge>
                ...
                <TableBody>
                  {aircraft.map((ac: Aircraft) => (
                    <TableRow key={ac.id}>
                      ...
                      <TableCell><Badge variant="secondary">{onDutyEmployees.filter((emp: Employee) => canAssign(emp, ac, 'B1')).length} available</Badge></TableCell>
                      <TableCell><Badge variant="secondary">{onDutyEmployees.filter((emp: Employee) => canAssign(emp, ac, 'B2')).length} available</Badge></TableCell>
